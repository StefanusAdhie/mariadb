  ...
  createConnection() {
    ...
    connection.connect(async (error) => {
      if (error) {
        ...
        this.connected = true;
        // reset count reconnect after success connect to database
        this.count_reconnect = 0;
        console.log({
      ...
      if (this.connection) {
        this.connection.query(query, data, async (error, result) => {
          if (error) {
            ...
            if (error.message.match(/ECONNRESET/i)) {
